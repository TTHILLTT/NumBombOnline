...
class Room(models.Model):
    name = models.CharField(_('room name'), max_length=127, default=_('New room'))  # 房间名称
    owner = models.ForeignKey(User, on_delete=models.CASCADE, blank=True, null=True, verbose_name=_('owner'))  # 房主
    ...
    answer = models.PositiveIntegerField(_('answer'), blank=True, null=True)  # 答案
    ...
    ready_players = models.ManyToManyField(User, related_name='room_ready_players', blank=True, verbose_name=_('ready players'))  # 准备好的玩家列表
    ...
    loser = models.ForeignKey(User, on_delete=models.CASCADE, blank=True, null=True, related_name='loser', verbose_name=_('loser'))  # 输者
    ...
    def save(self, *args, **kwargs):
        # 只有在创建新对象且answer为空时才生成答案
        if not self.pk and self.answer is None:
